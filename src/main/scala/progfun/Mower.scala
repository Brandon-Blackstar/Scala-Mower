...
case class Mower(start: Position, currentPosition: Position, direction: Direction, instructions: List[Instruction], end: Option[Position])

object Mower {

  def withEnd(start: Position, direction: Direction, instructions: List[Instruction], end: Position): Mower =
    Mower(start, start, direction, instructions, Some(end))

  def withoutEnd(start: Position, direction: Direction, instructions: List[Instruction]): Mower =
    Mower(start, start, direction, instructions, None)

  private def applyInstruction(mower: Mower, instruction: Instruction, lawn: Lawn): Mower = instruction match {
    case A => mower.copy(currentPosition = moveForward(mower.currentPosition, mower.direction, lawn))
    case G => mower.copy(direction = turnLeft(mower.direction))
    case D => mower.copy(direction = turnRight(mower.direction))
  }

  private def moveForward(point: Position, direction: Direction, lawn: Lawn): Position = direction match {
