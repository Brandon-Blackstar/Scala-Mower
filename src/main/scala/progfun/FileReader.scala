...
case class FileReader(filePath: String) {
  ...
  private def parsePosition(line: String): Either[LawnError, Position] = {
    ...
    if (parts.length == 2) {
      ...
      val y = parts(1)
      for {
        xPosition <- Try(x.toInt).toEither.left.map(_ => LawnError(s"Invalid x coordinate: $x"))
        yPosition <- Try(y.toInt).toEither.left.map(_ => LawnError(s"Invalid y coordinate: $y"))
  ...
  private def parseDirection(dir: String): Either[LawnError, Direction] = dir match {
    case "N" => Right(N)
    case "E" => Right(E)
    case "S" => Right(S)
    case "W" => Right(W)
  ...
  private def parseInstruction(inst: Char): Either[LawnError, Instruction] = inst match {
    case 'A' => Right(A)
    case 'G' => Right(G)
    case 'D' => Right(D)
  ...
  private def sequence(xs: List[Either[LawnError, Instruction]]): Either[LawnError, List[Instruction]] = xs match {
    case Nil => Right(Nil)
    case h :: t => h.flatMap(v => sequence(t).map(v :: _))
  }

  private def parseMowers(lines: List[String]): Either[LawnError, List[Mower]] =
    lines.grouped(2).toList.foldRight(Right(Nil): Either[LawnError, List[Mower]]) {
      case (List(pos, inst), acc) =>
        val posParts = pos.split(" ")
        val startPosString = s"${posParts(0)} ${posParts(1)}" // Concatenate the first two parts as position
