...
object CsvFileWriter {
  def toCsv(lawn: Lawn): String = {
    ...
    val mowers = lawn.mowers.zipWithIndex.map { case (mower, index) =>
      val start =
        s"${mower.start.x.toString};${mower.start.y.toString};${mower.direction.toString}"
      val end =
        s"${mower.currentPosition.x.toString};${mower.currentPosition.y.toString};${mower.direction.toString}"
